...
private val SPEED_KEY   = floatPreferencesKey("accessibility_speed")
...
class AccessibilityRepository(private val context: Context) {
    ...
    val speedFlow: Flow<Float> = context.accessibilityDataStore.data
        .map { prefs -> prefs[SPEED_KEY] ?: 0.90f }
    ...
    suspend fun saveSpeed(speed: Float) {
        context.accessibilityDataStore.edit { it[SPEED_KEY] = speed }
    }
...
class NutriTTS(context: Context, private var idioma: IdiomaVoz = IdiomaVoz.ESPANOL_MX) {
        ...
        tts = TextToSpeech(context) { status ->
            if (status == TextToSpeech.SUCCESS) {
                ...
                tts?.setOnUtteranceProgressListener(object : android.speech.tts.UtteranceProgressListener() {
                    ...
                    override fun onDone(utteranceId: String?) { liberarFocoAudio() }
                    override fun onError(utteranceId: String?) { liberarFocoAudio() }
                })
                ready = true
        ...
        try {
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
                ...
                val focusRequest = android.media.AudioFocusRequest.Builder(android.media.AudioManager.AUDIOFOCUS_GAIN_TRANSIENT_MAY_DUCK)
                    ...
                    .setOnAudioFocusChangeListener { /* No-op para accesibilidad */ }
                    .build()
    ...
    suspend fun hablarYEsperar(texto: String, margenMs: Long = 600L) {
        ...
        tts?.setOnUtteranceProgressListener(object : android.speech.tts.UtteranceProgressListener() {
            ...
            override fun onDone(id: String?)   { if (id == utteranceId) terminado = true; liberarFocoAudio() }
            override fun onError(id: String?)  { terminado = true; liberarFocoAudio() }
        })
...
fun vibrateTap(haptic: HapticFeedback)     {
    haptic.performHapticFeedback(HapticFeedbackType.TextHandleMove)
    NutriEarcons.playButtonHover()
}
fun vibrateSuccess(haptic: HapticFeedback) {
    haptic.performHapticFeedback(HapticFeedbackType.LongPress)
    NutriEarcons.playSuccess()
}
fun vibrateError(haptic: HapticFeedback)   {
    haptic.performHapticFeedback(HapticFeedbackType.LongPress)
    NutriEarcons.playError()
}

fun vibrateTap(context: Context?)     {
    MotorHapticoNutriIA.vibrarFuerte(context, 35L, 190)
    NutriEarcons.playButtonHover()
}
fun vibrateSuccess(context: Context?) {
    MotorHapticoNutriIA.vibrarFuerte(context, 65L, 255)
    NutriEarcons.playSuccess()
}
fun vibrateError(context: Context?)   {
    MotorHapticoNutriIA.vibrarFuerte(context, 85L, 255)
    NutriEarcons.playError()
}
